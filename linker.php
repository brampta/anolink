...
//	$result = mysql_query("/*anolink linker*/ SELECT link, linkname FROM user_links WHERE user_linkID = '$addslashed_getli'");
...
//
//	echo '<html><head>
//<title>'.$main_site_name.'</title>
//<link rel="shortcut icon" href="http://'.$main_domain_name.'/common_files/pics/tape4rev.ico" />
//<link rel="stylesheet" type="text/css" href="http://'.$main_domain_name.'/common_files/crapolister.css" />
...
//		<br /><br />You are now leaving '.$main_site_name.'.
//			</div>
//
//			<br /><br />
//
...
//add a click
mysql_query("/*anolink linker*/ UPDATE user_links SET clicks = clicks+1 WHERE user_linkID = '$addslashed_getli'");
//get link values
$result = mysql_query("/*anolink linker*/ SELECT link, bannerURL, linkname FROM user_links WHERE user_linkID = '$addslashed_getli'");
while($row = mysql_fetch_array($result))
{
	$thislink=trim($row['link']);
	$startofthislink=substr($thislink,0,7);
	if($startofthislink!='http://')
	{$thislink='http://anolink.com/nab/'.$thislink;}
	if($_GET['go']=='yes')
	{$link=$thislink;}
